...
uint64_t read_filedata(struct file* file1, uint8_t* buffer) {
    if (file1->attributes & 0b10000000) {
        ...
        for (uint32_t i = 0; i < 61; i++) {
            ...
            lread_sector(atufsinfo.cluster0+(e.startcluster*blockinsec), (uint16_t*)buffer, totalcsectors);
        }
    ...
    for (uint32_t i = 0; i < clusteramount;i++) {
        ...
        if (sector != currentsector) {
            if (currentsector != (uint32_t)-1) {
                write_sector(atufsinfo.startbmpcluster+currentsector, atufsbuffer1, 1);
            }
            read_sector(atufsinfo.startbmpcluster+sector, atufsbuffer1, 1);
            currentsector = sector;
        }
        buffer[byte] &= ~(1 << localb);
    }
    atufsinfo.alocated_clusters -= clusteramount;
    write_sector(0, (uint16_t*)&atufsinfo, 1);
    write_sector(atufsinfo.startbmpcluster+currentsector, atufsbuffer1, 1);
}
...
void write_file(struct file* f, uint32_t filenum, uint8_t *buffer, uint64_t buffer_size) {
    ...
    if (buffer_size <= 490) {
        ...
        if (nfilesize_inclus == oldfilesize_inclus) {
            for (int i = 0; i < 61; i++) {
                ...
                lwrite_sector(atufsinfo.cluster0+e.startcluster, (uint16_t*)buffer, e.manyclusters);
            ...
            for (int i = 0; i < 61;i++) {
                ...
                if (e.manyclusters) {
                    if (e.manyclusters > clusters_left) {
                        ...
                        lwrite_sector(atufsinfo.cluster0+e.startcluster, (uint16_t*)buffer, clusters_left);
                        break;
                    ...
                    lwrite_sector(atufsinfo.cluster0+copy.extents[i].startcluster, (uint16_t*)buffer, copy.extents[i].manyclusters);
    ...
    write_sector(atufsinfo.file0+filenum, (uint16_t*)&copy, 1);
}